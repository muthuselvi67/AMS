...
const Tasks = () => {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                setMembers((usr.data.data || []).filter(u => u.role === 'developer' || u.role === 'pm'));
            } catch { }
        };
        init(); fetchTasks();
    }, []);
    ...
    const openEdit = (t) => {
        setForm({
            name: t.name, project: t.project?.id || '', assignedTo: t.assignedTo?.id || '',
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            else { await api.put(`/tasks/${modal.item.id}`, form); toast.success('Task updated!'); }
    ...
    const updateStatus = async (task, status) => {
        try { await api.put(`/tasks/${task.id}`, { status }); fetchTasks(); }
                            ...
                            <tbody>
                                {loading ? Array(5).fill(0).map((_, i) => <tr key={i}>{Array(7).fill(0).map((_, j) => <td key={j}><div className="skeleton skeleton-text" /></td>)}</tr>) :
                                    filtered.length === 0 ? <tr><td colSpan={7}><div className="empty-state"><div className="empty-state-icon" style={{ color: 'var(--text-muted)' }}><ListTodo size={48} /></div><h3>No tasks found</h3></div></td></tr> :
                                        filtered.map(t => (
                                            <tr key={t.id} style={{ background: isOverdue(t) ? '#FEF2F2' : undefined }}>
                                                ...
                                                <td><div style={{ display: 'flex', gap: 6 }}>
                                                    {(user?.role === 'admin' || user?.role === 'pm') && <>
                                                        ...
                                                        <button className="btn btn-danger btn-sm btn-icon" onClick={() => handleDelete(t.id)}><Trash2 size={13} /></button>
                                                    </>}
                                                </div></td>
                                            </tr>
                                        ))}

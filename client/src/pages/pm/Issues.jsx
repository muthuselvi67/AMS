...
const Issues = () => {
    ...
    useEffect(() => {
        const init = async () => {
            const [p, u] = await Promise.all([api.get('/projects').catch(() => ({ data: { data: { projects: [] } } })), api.get('/users').catch(() => ({ data: { users: [] } }))]);
            setProjects(p.data.data?.projects || []);
            setMembers((u.data.data || []).filter(u => u.role !== 'client'));
        };
        init(); fetchIssues();
    }, []);
    ...
    const openEdit = (issue) => {
        setForm({ project: issue.project?.id || '', title: issue.title, description: issue.description || '', severity: issue.severity, assignedTo: issue.assignedTo?.id || '', status: issue.status });
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            else { await api.put(`/issues/${modal.item.id}`, form); toast.success('Issue updated!'); }
                                        ...
                                        return (
                                            <tr key={issue.id}>
                                                ...
                                                <td><div style={{ display: 'flex', gap: 6 }}>
                                                    ...
                                                    {(user?.role === 'admin' || user?.role === 'pm') && <button className="btn btn-danger btn-sm btn-icon" onClick={() => handleDelete(issue.id)}><Trash2 size={13} /></button>}
                                                </div></td>
                                            </tr>
                                        );
                                    })}
                    ...
                    <div className="form-row">
                        <div className="form-group"><label className="form-label">Assign To</label>
                            <select className="form-control" value={form.assignedTo} onChange={e => setForm(f => ({ ...f, assignedTo: e.target.value }))}>
                                <option value="">Unassigned</option>{members.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                            </select></div>

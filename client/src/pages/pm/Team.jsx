...
const Team = () => {
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                setUsers(u.data.data.filter(u => u.isActive));
    ...
    const getUserTaskCount = (userId) => tasks.filter(t => t.assignedTo?.id === userId || t.assignedTo === userId).length;
    const getUserActiveCount = (userId) => tasks.filter(t => (t.assignedTo?.id === userId || t.assignedTo === userId) && t.status === 'in-progress').length;
            ...
            {loading ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 16 }}>
                    {filtered.map(u => {
                        ...
                        return (
                            <div key={u.id} className="card" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 14 }}>

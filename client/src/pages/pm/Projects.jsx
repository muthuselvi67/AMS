...
const Projects = () => {
    ...
    const openEdit = (p) => {
        setForm({
            ...
            description: p.description || '', status: p.status, assignedPM: p.assignedPM?.id || '',
            tags: (p.tags || []).join(', ')
        });
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            else { await api.put(`/projects/${modal.item.id}`, payload); toast.success('Project updated!'); }
            ...
            {loading ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 16 }}>
                    {filtered.map(p => {
                        ...
                        return (
                            <div key={p.id} className="card" style={{ padding: '18px 20px', display: 'flex', flexDirection: 'column', gap: 12, transition: 'transform 0.2s', cursor: 'pointer' }}
                                ...
                                <div style={{ display: 'flex', gap: 8, borderTop: '1px solid var(--border)', paddingTop: 10 }}>
                                    ...
                                    <div style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
                                        <button className="btn btn-secondary btn-sm btn-icon" onClick={(e) => { e.stopPropagation(); navigate(`/pm/projects/${p.id}`); }}><Eye size={13} /></button>
                                        {(user?.role === 'admin' || user?.role === 'pm') && <>
                                            <button className="btn btn-secondary btn-sm btn-icon" onClick={(e) => { e.stopPropagation(); openEdit(p); }}><Edit size={13} /></button>
                                            {user?.role === 'admin' && <button className="btn btn-danger btn-sm btn-icon" onClick={(e) => { e.stopPropagation(); handleDelete(p.id); }}><Trash2 size={13} /></button>}
                                        </>}
                                    </div>
                                </div>
                            </div>

...
const Risks = () => {
    ...
    const openEdit = (r) => {
        setForm({ project: r.project?.id || '', description: r.description, level: r.level, impact: r.impact || '', mitigationPlan: r.mitigationPlan || '', responsiblePerson: r.responsiblePerson?.id || '', status: r.status });
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            else { await api.put(`/risks/${modal.item.id}`, form); toast.success('Risk updated!'); }
                ...
                {loading ? Array(4).fill(0).map((_, i) => <div key={i} className="card skeleton" style={{ height: 100 }} />) :
                    risks.length === 0 ? <div className="card"><div className="empty-state"><div className="empty-state-icon" style={{ color: 'var(--text-muted)' }}><ShieldAlert size={48} /></div><h3>No risks identified</h3><p>Add risks to help manage project health</p></div></div> :
                        risks.map(r => {
                            ...
                            return (
                                <div key={r.id} className="card" style={{ padding: '16px 20px', borderLeft: `4px solid ${lm.color}` }}>

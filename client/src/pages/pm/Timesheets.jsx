...
const Timesheets = () => {
    ...
    const fetchLogs = async () => {
        ...
        try {
            const params = {};
            if (filterProject) params.project_id = filterProject; // backend expects project_id
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                if (user?.role === 'admin' || user?.role === 'pm' || user?.role === 'hr') {
                    const u = await api.get('/users');
                    setUsers(u.data.data?.filter(usr => usr.isActive) || []);
                }
            } catch { }
        };
        init(); fetchLogs();
    }, []);
    useEffect(() => { fetchLogs(); }, [filterProject, filterUser]);
    ...
    const handleSubmit = async (e) => {
        ...
        setSaving(true);
        try {
            // Backend expects project_id and task_id
            await api.post('/timelogs', {
                project_id: form.project,
                task_id: form.task || null,
                date: form.date,
    ...
    const projectTasks = tasks.filter(t => t.project?.id === form.project || t.project === form.project);
    ...
    return (
        <div className="fade-in">
            ...
            <div className="filter-bar">
                ...
                {(user?.role === 'admin' || user?.role === 'pm' || user?.role === 'hr') && (
                    <select className="form-control" style={{ width: 220 }} value={filterUser} onChange={e => setFilterUser(e.target.value)}>
                                            ...
                                            <td>
                                                <div style={{ display: 'flex', gap: 6 }}>
                                                    {(user?.role === 'admin' || user?.role === 'pm') && !log.isApproved && (
                                                        <button className="btn btn-primary btn-sm" onClick={() => approveLog(log.id)} style={{ fontSize: 11 }}> Approve</button>
                                                    )}
                                                    {!log.isApproved && (
                                                        <button className="btn btn-danger btn-sm btn-icon" onClick={() => deleteLog(log.id)} title="Delete"><Trash2 size={13} /></button>
                                                    )}
                                                </div>
                                            </td>
                                        </tr>

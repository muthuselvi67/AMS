...
const LEVEL_MAP = {
    1: { label: 'Level 1 — Outstanding',            color: '#10B981', rgba: 'rgba(16, 185, 129, 0.12)' },
    2: { label: 'Level 2 — Exceeds Expectations',    color: '#4F9CF9', rgba: 'rgba(79, 156, 249, 0.12)' },
    3: { label: 'Level 3 — Meets Expectations',      color: '#F59E0B', rgba: 'rgba(245, 158, 11, 0.12)'  },
    4: { label: 'Level 4 — Needs Improvement',       color: '#EF4444', rgba: 'rgba(239, 68, 68, 0.12)'   },
};
const STATUS_META = {
    draft:          { label: 'Draft',         color: '#94A3B8', rgba: 'rgba(148, 163, 184, 0.12)' },
    submitted:      { label: 'Submitted',     color: '#F59E0B', rgba: 'rgba(245, 158, 11, 0.12)'  },
    'under-review': { label: 'Under Review',  color: '#4F9CF9', rgba: 'rgba(79, 156, 249, 0.12)'  },
    'hr-review':    { label: 'HR Review',     color: '#8B5CF6', rgba: 'rgba(139, 92, 246, 0.12)'  },
    approved:       { label: 'Approved ✅',   color: '#10B981', rgba: 'rgba(16, 185, 129, 0.12)'  },
    rejected:       { label: 'Rejected',      color: '#EF4444', rgba: 'rgba(239, 68, 68, 0.12)'   },
};
...
const PMAppraisalEmployee = () => {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const { data } = await api.put(`/pmappraisals/${appraisal.id}`, form);
            setAppraisal(data.appraisal);
            toast.success('Saved!');
    ...
    const handleSubmit = async () => {
        ...
        try {
            await handleSave();
            const { data } = await api.post(`/pmappraisals/${appraisal.id}/submit`);
            setAppraisal(data.appraisal);
        ...
        <div className="fade-in">
            ...
            {loading ? <div className="card skeleton" style={{ height: 200 }} /> :
                ...
                !appraisal ? (
                    ...
                    <>
                        {/* Status Banner */}
                        <div className="card" style={{ padding: '14px 20px', marginBottom: 20, background: sm.rgba, border: `1.5px solid ${sm.color}55`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <div>
                            ...
                            {finalLevel && <div style={{ textAlign: 'right' }}><div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Final Result</div><div style={{ fontWeight: 800, color: LEVEL_MAP[finalLevel]?.color, fontSize: 15, background: LEVEL_MAP[finalLevel]?.rgba, padding: '2px 10px', borderRadius: 6, display: 'inline-block' }}>{LEVEL_MAP[finalLevel]?.label}</div></div>}
                        </div>

                        {/* HR Remarks (on approval) */}
                        {appraisal.status === 'approved' && appraisal.hrReview?.hrRemarks && (
                            <div className="card" style={{ background: 'rgba(16, 185, 129, 0.12)', border: '1.5px solid rgba(16, 185, 129, 0.35)', marginBottom: 20, padding: '14px 20px' }}>
                        ...
                        {['hr-review', 'approved'].includes(appraisal.status) && appraisal.managerReview?.categoryLevel && (
                            <div className="card" style={{ marginBottom: 20 }}>
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                                    <div><div style={{ fontSize: 11, color: 'var(--text-muted)' }}>Category Level</div><div style={{ fontWeight: 800, color: LEVEL_MAP[mgrLevel]?.color, fontSize: 14, background: LEVEL_MAP[mgrLevel]?.rgba, padding: '2px 10px', borderRadius: 6, display: 'inline-block', marginTop: 2 }}>{LEVEL_MAP[mgrLevel]?.label}</div></div>

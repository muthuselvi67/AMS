...
const Directory = () => {
    ...
    const fetchEmployees = async () => {
        ...
        try {
            ...
            setEmployees(data.data.filter(u => u.isActive));
    ...
    const departments = ['HR', 'Administration', 'Training', 'Admin', 'Development', 'Manager'];
    ...
    return (
        <div className="fade-in">
            ...
            {loading ? (
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 }}>
                    {employees.map(emp => (
                        <div key={emp.id} className="card" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: 12, transition: 'transform 0.2s, box-shadow 0.2s' }}
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                                ...
                                {emp.phone && <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-muted)' }}><Phone size={13} />{emp.phone} (Pri)</div>}
                                {emp.phoneSecondary && <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: 'var(--text-muted)' }}><Phone size={13} />{emp.phoneSecondary} (Sec)</div>}
                            </div>

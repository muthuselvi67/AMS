...
const LeaveTypes = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            const payload = {
                ...
                max_carry_forward: form.maxCarryForward,
                is_paid: form.isPaid,
                requires_approval: form.requiresApproval
            };
            if (modal.mode === 'create') { await api.post('/leave-types', payload); toast.success('Leave type created!'); }
            else { await api.put(`/leave-types/${modal.item.id}`, payload); toast.success('Leave type updated!'); }
            close(); fetch();
    ...
    const handleDelete = async (item) => {
        if (!window.confirm(`Are you sure you want to permanently delete "${item.name}"? This cannot be undone.`)) return;
        try {
            await api.delete(`/leave-types/${item.id}`);
        ...
        <div className="fade-in">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(300px,1fr))', gap: 16 }}>
                {loading ? Array(6).fill(0).map((_, i) => <div key={i} className="skeleton skeleton-card" />) :
                    types.length === 0 ? <div className="empty-state"><div className="empty-state-icon" style={{ color: 'var(--text-muted)' }}><CalendarCheck size={48} /></div><h3>No leave types</h3></div> :
                        types.map(lt => (
                            <div key={lt.id} className="card">
                                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 12 }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 6 }}>
                                        ...
                                        <button className="btn btn-ghost btn-sm btn-icon" onClick={() => handleDelete(lt)} title="Delete leave type" style={{ color: 'var(--danger)' }}>
                                            <Trash2 size={14} />
                                        </button>
                                    </div>
                                </div>

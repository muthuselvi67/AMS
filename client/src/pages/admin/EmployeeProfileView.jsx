...
import { Briefcase, Calendar, ChevronLeft, Edit, Save, X, GraduationCap, Globe, Github, Linkedin, FileText, BarChart2, Clock, TrendingUp, Award } from 'lucide-react';
...
const EmployeeProfileView = () => {
  ...
  const handleIdCardPhotoChange = async (e) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        await api.put(`/users/${id}/profile`, { ...form, idCardPhoto: newPhoto });
  ...
  const handleCoverChange = (e) => {
    ...
    reader.onloadend = async () => {
      ...
      try {
        await api.put(`/users/${id}/profile`, { ...form, coverPhoto: reader.result });
        setEmployee(prev => ({ ...prev, coverPhoto: reader.result }));
  ...
  // Generate full month range: 2026-01 to 2036-12
  const rangeMonths = [];
  ...
  // Filter attendance by selectedMonth if selectedMonth is set
  ...
  const effectivePresent = presentDays + lateDays + wfhDays; // present + late + wfh = counted as attended
  ...
  return (
    <div className="fade-in" style={{ paddingBottom: 40 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div>
            ...
            <p style={{ color: 'var(--text-muted)', margin: 0, fontSize: 13, fontWeight: 500, marginTop: 2 }}>
              {isEditing ? 'Update employee details' : 'Comprehensive view of employee information'}
            </p>
          </div>
        </div>
      ...
      {/* ── Main 2-column layout: LEFT = info | RIGHT = profile card ── */}

                                    ...
                                    <tr>
                                        ...
                                        <td style={{ textAlign: 'right' }}>
                                            <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                                                <button
                                                    onClick={() => editId === p.id ? cancelEdit() : startEdit(p)}
                                                    className={`icon-btn ${editId === p.id ? 'danger' : 'warning'}`}
                                                    title={editId === p.id ? 'Cancel Edit' : 'Manual Entry'}
                                                >
                                                    {editId === p.id ? <X size={16} /> : <Pencil size={16} />}
                                                </button>
                                                {p.status !== 'Paid' && (
                                                    <button onClick={() => updateStatus(p.id, 'Paid')} className="icon-btn success" title="Mark as Paid">

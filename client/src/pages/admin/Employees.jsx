...
const Employees = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (modal.mode === 'create') {
                ...
                if (password) await api.put(`/users/${modal.user.id}/password`, { password });
    ...
    const toggleStatus = async (user) => {
        try {
            if (user.isActive) {
                await api.put(`/users/${user.id}/status`, { isActive: false });
                                                    ...
                                                    <td>
                                                        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                                                            ...
                                                            {(user?.role === 'admin' || (user?.role === 'hr' && emp.role === 'employee')) ? (
                                                                <>
                                                                    <button className="btn-icon-circle view" onClick={() => navigate(`/admin/employees/${emp.id}`)} title="View"><Eye size={14} strokeWidth={2.5} /></button>

...
const typeColors = { 
    ...
    company: 'badge-approved', 
    optional: 'badge-on-leave',
    government: 'badge-government',
    bank: 'badge-bank',
    working_saturday: 'badge-working-saturday',
    floating_leave: 'badge-floating-leave'
};
...
const Holidays = () => {
    ...
    const fetch = async () => {
        setLoading(true);
        try { const { data } = await api.get('/holidays', { params: { year: new Date().getFullYear() } }); setHolidays(data.data); }
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            else { await api.put(`/holidays/${modal.item.id}`, form); toast.success('Holiday updated!'); }
            close(); fetch();
                                ...
                                holidays.length === 0 ? <tr><td colSpan={6}><div className="empty-state"><div className="empty-state-icon" style={{ color: 'var(--text-muted)' }}><Palmtree size={48} /></div><h3>No holidays added</h3></div></td></tr> :
                                    holidays.map(h => (
                                        <tr key={h.id}>
                                            ...
                                            <td><span className={`badge ${typeColors[h.type] || 'badge-pending'} `}>{formatType(h.type)}</span></td>
                                            ...
                                            <td><div style={{ display: 'flex', gap: 6 }}>
                                                ...
                                                <button className="btn btn-danger btn-sm btn-icon" onClick={() => handleDel(h.id)}><Trash2 size={14} /></button>
                                            </div></td>
                                        </tr>
                                    ))
                            }
                ...
                <form onSubmit={handleSave}>
                    ...
                    <div className="form-group"><label className="form-label">Type</label>
                        <select name="type" className="form-control" value={form.type} onChange={handleChange}>
                            ...
                            <option value="floating_leave">Floating Leave</option>
                        </select>
                    </div>

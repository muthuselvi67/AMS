...
const Announcements = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (modal.mode === 'create') {
                ...
                await api.put(`/announcements/${modal.item.id}`, form);
    ...
    const togglePin = async (item) => {
        try {
            await api.put(`/announcements/${item.id}`, { ...item, pinned: !item.pinned, postedBy: item.postedBy?.id });
            fetchAnnouncements();
            ...
            {loading ? (
                ...
                <div style={{ display: 'grid', gap: 14 }}>
                    ...
                    {filtered.filter(a => a.pinned).concat(filtered.filter(a => !a.pinned)).map(item => {
                        ...
                        return (
                            <div key={item.id} className="card" style={{ border: item.pinned ? `2px solid ${meta.color}` : '1px solid var(--border)', padding: '18px 20px' }}>
                                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                                    ...
                                    <div style={{ display: 'flex', gap: 6, flexShrink: 0, flexDirection: 'column', alignItems: 'flex-end' }}>
                                        ...
                                        <button className="btn btn-danger btn-sm btn-icon" onClick={() => handleDelete(item.id)} title="Delete"><Trash2 size={14} /></button>
                                    </div>
                                </div>
                            </div>
                        );

...
const Appraisal = () => {
    ...
    const fetchAppraisals = async () => {
        ...
        try {
            ...
            // API returns { status: true, message: "...", data: { appraisals: [...] } }
            setAppraisals(res.data?.data?.appraisals || []);
        } catch (err) { 
            console.error(err);
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (modal.mode === 'create') {
                ...
                await api.put(`/appraisals/${modal.item.id}`, form);
                    ...
                    <table style={{ minWidth: 850 }}>
                        <thead>
                            <tr>
                                ...
                                <th style={{ minWidth: 220 }}>Ratings Breakdown</th>
                                <th style={{ minWidth: 100 }}>Avg Rating</th>
                        ...
                        <tbody>
                            {loading ? (
                                ...
                                appraisals.map(a => (
                                    <tr key={a.id}>
                                        <td>
                                            <div style={{ fontWeight: 600, color: 'var(--text-primary)', fontSize: 13, whiteSpace: 'nowrap' }}>{a.employee?.name || 'Unknown'}</div>
                                            <div style={{ fontSize: 11, color: 'var(--text-muted)', whiteSpace: 'nowrap' }}>{a.employee?.department || ''}</div>
                                        </td>
                                        <td style={{ fontWeight: 600, fontSize: 13, whiteSpace: 'nowrap' }}>{a.period}</td>
                                        <td>
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, minWidth: 200 }}>
                                                {CRITERIA.map(c => {
                                                    const val = (a.ratings && a.ratings[c]) ? a.ratings[c] : 0;
                                                    const pct = (val / 5) * 100;
                                                    return (
                                                        <div key={c}>
                                                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 3, gap: 12 }}>
                                                                <span style={{ fontSize: 10, fontWeight: 600, color: 'var(--text-secondary)', textTransform: 'capitalize', whiteSpace: 'nowrap' }}>
                                                                    {CRITERIA_LABELS[c]}
                                                                </span>
                                                                <span style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 600 }}>{val}/5</span>
                                                            </div>
                                        ...
                                        <td>
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontWeight: 700, color: '#F59E0B', fontSize: 16, whiteSpace: 'nowrap' }}>
                                                ...
                                                {a.avgRating ? Number(a.avgRating).toFixed(1) : avgRating(a.ratings).toFixed(1)}
                                        ...
                                        <td>
                                            <div style={{ display: 'flex', gap: 6 }}>
                                                ...
                                                <button className="btn btn-danger btn-sm btn-icon" onClick={() => handleDelete(a.id)} title="Delete"><Trash2 size={14} /></button>
                                            </div>
                                        </td>
                                    </tr>
                                ))
                    ...
                    <div className="form-row">
                        <div className="form-group">
                            ...
                            <select className="form-control" value={form.employee} onChange={e => setForm(f => ({ ...f, employee: e.target.value }))} required disabled={modal.mode === 'edit'}>
                                ...
                                {employees.map(emp => <option key={emp.id} value={emp.id}>{emp.name}  {emp.department || 'N/A'}</option>)}
                            </select>
                        </div>
                        <div className="form-group">
